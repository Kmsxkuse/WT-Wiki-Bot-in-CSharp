...
namespace WT_Wiki_Bot_in_CSharp {
    internal static class ExportArmorPen {
        static string[] _colourValues = new [] { 
...
<div style=""position:absolute;left:{Math.Round(tableSize / 10, 2)}rem;top:{Math.Round(tableSize / 20, 2)}rem;width:{intSize}rem;height:{intSize}rem;background:white;border:solid;"">";
            ...
            maxPen = (float)(Math.Round(maxPen / (maxLines - 1)) * (maxLines - 1));
            
            var chartUnits = new StringBuilder();
            for (var lineNum = maxLines; lineNum > 0; lineNum--) {
                // Y Values
                chartUnits.Append($@"<div style=""position:absolute;top:{Math.Round((double) intSize / maxLines * (lineNum + 0.5), 2)}rem;width:{Math.Round(tableSize / 12, 2)}rem"" align=""right"">{Math.Round(maxPen / (maxLines - 1) * (maxLines - lineNum), 1)}</div>");
                
                // X Values
                chartUnits.Append($@"<div style=""position:absolute;top:{Math.Round(intSize + tableSize * 3 / 40 - (decimal) 0.3, 2)}rem;left:{Math.Round((double) intSize / maxLines * lineNum + 0.6, 2)}rem;width:{Math.Round(tableSize / lineNum, 2)}"" align=""center"">{(lineNum - 1) * 100}</div>");
            }
            ...
            for (var bullet = 0; bullet < armorPowerArr.Count; bullet++) {
                for (var coordinate = 0; coordinate < armorPowerArr[bullet].Count; coordinate++) {
                    ...
                    if (coordinate != 0) {
                        ...
                        // Lines
                        penLines.Append($@"<div style=""position:absolute;transform:rotate({rotate}rad);left:{(boxPointX + prevPoint[0]) / 2}%;bottom:{(boxPointY + prevPoint[1]) / 2 - lineLength / 2 + 1}%;height:{lineLength}%;border-right:solid #{_colourValues[bullet]}""></div>
");
                    }
                    ...
                    // Points
                    penCoords.Append($@"<div style=""position:absolute;left:{boxPointX}%;bottom:{boxPointY}%;width:{boxSize}rem;height:{boxSize}rem;border:solid #{_colourValues[bullet]};border-radius:50%;background:white;""></div>");
                }
                ...
                // Legend
                penLegend.Append($@"<div style=""position:relative;padding:0.2rem;border:solid #{_colourValues[bullet]};width:20%;top:1%;margin:0 0 0 75%;background:white;text-align:center"">{cleanedName}</div>");
            }
...
<div style=""position:absolute;width:100%;top:{tableSize - (decimal) 1.5}rem"" align=""center"">Distance in meters</div>
<div style=""transform:rotate(90deg);position:absolute;left:-{tableSize / 2 - 1}rem;width:100%;"" align=""right"">Penetration in mm</div></b>";
